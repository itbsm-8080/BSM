...
uses
  ...
  frxDMPExport, dxSkinDarkRoom, dxSkinFoggy, dxSkinSeven, dxSkinSharp,
  MyAccess;
...
type
  TfrmBPB = class(TForm)
    ...
    clRegister: TcxGridDBColumn;
    clKondisi: TcxGridDBColumn;
...
begin
  ...
  begin
    ...
    zAddField(FCDS, 'register', ftString, False, 10);
    zAddField(FCDS, 'kondisi', ftString, False, 10);
    zAddField(FCDS, 'status', ftString, False, 10);
    FCDS.CreateDataSet;
  end;
  Result := FCDS;
end;
...
ExecSQLDirect(frmMenu.conn, s);
  ...
  begin
   ...
   begin
    S:='insert into tbpb_DTL (bpbd_bpb_nomor,bpbd_brg_kode,bpbd_brg_satuan,bpbd_qty,bpbd_tgl_expired,bpbd_nourut,'
      + ' bpbd_status,bpbd_register,bpbd_kondisi,bpbd_status2,bpbd_idbatch) values ('
      + Quot(edtNomor.Text) +','
      ...
      + Quot(CDS.FieldByName('register').AsString) +','
      + Quot(CDS.FieldByName('kondisi').AsString) +','
       ...
       begin
         ...
         begin
            ...
            begin
                      ...
                      CDS.FieldByName('Register').AsString    := 'Ada';
                      CDS.FieldByName('Kondisi').AsString     := 'Baik';
                      CDS.FieldByName('Status').AsString      := 'Terima';
                      CDS.Post;
                   i:=i+1;
                   next;
            end ;
...
begin
  ...
  s := ' select BPB_NOMOr,bpb_tanggal,po_nomor,bpb_memo,po_sup_kode,bpb_gdg_kode,'
     ...
     + ' bpbd_register,bpbd_kondisi,bpbd_status2 ,bpbd_idbatch'
         ...
         begin
            ...
            while  not Eof do
             begin
                      ...
                      CDS.FieldByName('register').AsString      := fieldbyname('bpbd_register').Asstring;
                      CDS.FieldByName('kondisi').AsString      := fieldbyname('bpbd_kondisi').Asstring;
                      CDS.FieldByName('status').AsString      := fieldbyname('bpbd_status2').Asstring;
