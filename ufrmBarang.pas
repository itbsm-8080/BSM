...
type
  TfrmBarang = class(TForm)
    ...
    Label12: TLabel;
    Label16: TLabel;
    edtijinedar: TAdvEdit;
    edtprodusen: TAdvEdit;
    dtberlaku: TDateTimePicker;
    AdvPanel6: TAdvPanel;
    cxLabel1: TcxLabel;
  ...
  try
    ...
    begin
      ...
      edtNama.Text := fieldbyname('brg_nama').AsString;
       ...
       edtijinedar.Text := fieldbyname('brg_ijinedar').AsString;
       edtprodusen.Text := fieldbyname('brg_produsen').AsString;
       dtberlaku.DateTime := fieldbyname('brg_berlaku').AsDateTime;
...
if FLAGEDIT then
  s:='update tbarang set '
    ...
    + ' brg_ijinedar = '+ Quot(edtijinedar.Text)+','
    + ' brg_produsen = '+ Quot(edtprodusen.Text) +','
    + ' brg_berlaku = '+ QuotD(dtberlaku.DateTime)+','
...
begin
  ...
  s :=  ' insert into tbarang '
             ...
             + ' brg_min_stok,brg_max_stok,brg_hrgjual,brg_hrgbeli,brg_lastcost,brg_isaktif,brg_isstok,brg_isexpired,brg_isproductfocus,'
             + ' brg_ijinedar,brg_produsen,brg_berlaku,date_create,user_create'
             + ' ) '
             + ' values ( '
             + Quot(edtKode.Text) + ','
             + Quot(edtNama.Text) + ','
             ...
             + Quot(edtijinedar.Text)+','
             + Quot(edtprodusen.Text) +','
             + QuotD(dtberlaku.DateTime) +','
...
procedure TfrmBarang.HapusBaris1Click(Sender: TObject);
begin
 If CDS2.Eof then exit;
  CDS2.Delete;
//  If CDS2.Eof then initgrid;
end;
