...
begin
  Self.SQLMaster := 'select Nomor,Nobukti,Tanggal,Jthtempo,TanggalSuplier,Memo,Supplier,NoPajak,Total,Retur,Ppn,(total-ifnull(ppn,0))-(ifnull(retur,0)/(if(ppn>0,if(tanggal<"2022/04/01",1.1,1.11),1))) Dpp,Bayar,Status_Bayar,Pajak '
                  + ' from (select inv_nomor Nomor,inv_nobukti Nobukti,inv_tanggal Tanggal ,inv_tglbukti TanggalSuplier ,inv_jthtempo Jthtempo,inv_Memo Memo ,sup_nama  Supplier, '
                  + ' inv_nopajak NoPajak,inv_amount Total,'
                  + ' (select ifnull(sum(ret_amount),0) from tret_hdr where ret_inv_nomor=inv_nomor) Retur ,'
