...
begin
  ...
  Self.SQLMaster := 'select brg_kode Kode,brg_nama Nama,Kategori,'
          ...
          + ' and mst_noreferensi like "RI%" and mst_tanggal <= '+ QuotD(enddate.DateTime)
          + ' order by mst_tanggal desc LIMIT 1) Lastcost'
          ...
          + '  ) final '
          + ' where ifnull(saldoawal,0) > 0 or ifnull(qty_in,0) > 0 or ifnull(qty_OUT,0) > 0 or ifnull(saldoawal,0) '
          + ' order by brg_kode' ;
 ...
 Self.SQLDetail := 'select mst_brg_kode Kode,mst_noreferensi Nomor,mst_tanggal Tanggal,'
              ...
              + ' if (mst_noreferensi like "%RET%","Retur Pembelian",if (mst_noreferensi like "%KOR%","Koreksi",if (mst_noreferensi like "%MUS%","Pemusnahan","Pembelian")))))) Keterangan ,'
