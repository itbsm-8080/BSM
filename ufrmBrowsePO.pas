...
begin
  Self.SQLMaster := 'select po_nomor Nomor,po_tanggal Tanggal ,po_Memo Memo ,sup_nama  Supplier,if(po_isclosed=0,"Belum","Sudah") Closed, '
                  + ' po_amount Total,po_taxamount Ppn,if(po_status_rec=0,"Belum","Sudah") Receipt,po_pemesan Pemesan'
                  + ' from tpo_hdr'
...
procedure TfrmBrowsePO.cxButton5Click(Sender: TObject);
begin
  inherited;
    frmPO.doslipPO2(CDSMaster.FieldByname('Nomor').AsString);
end;
