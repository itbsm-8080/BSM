...
begin
  ...
  Self.SQLDetail := 'select JUR_NO Nomor,JURD_REK_KODE Account,rek_nama AccountName, jurd_debet Debet,jurd_kredit Kredit,cc_nama CostCenter,kar_nama NamaKaryawan,nama Unit from tjurnal'
                    ...
                    + ' LEFT JOIN tkaryawanall ON kar_nik=jurd_kar_nik'
                    + ' LEFT JOIN tunit ON id =kar_id_unit'
                    + ' where jur_tipetransaksi='+ quot('Penerimaan Lain')
                    + ' and jur_tanggal between ' + QuotD(startdate.DateTime) + ' and ' + QuotD(enddate.DateTime)
