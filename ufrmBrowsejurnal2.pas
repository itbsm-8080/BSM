...
type
  TfrmBrowseJurnal2 = class(TForm)
    ...
    sqljurnal: TMyQuery;
    cdsjurnal: TClientDataSet;
    dsJurnal: TDataSource;
    dspjurnal: TDataSetProvider;
...
begin
      ...
      s:= 'select month(Tanggal) Bulan,year(Tanggal) Tahun,Tanggal,Nomor,Referensi,Account,AccountName,Keterangan,Debet,Kredit,Kelompok,CostCenter,Nopol,'
      + '(SELECT cbg_nama FROM tcabang WHERE cbg_kode=LEFT(nomor,2)) Cabang '
      + ' from bsm.alljurnal ' 
      + ' having Tanggal between ' + QuotD(startdate.DateTime) + ' and ' + QuotD(enddate.DateTime)
      + ' order by nomor ';


      cdsjurnal.Close;
        sqljurnal.Connection := frmmenu.conn;
        sqljurnal.SQL.Text := s;
        cdsjurnal.open;
        ...
        Skolom :='Bulan,Tahun,Tanggal,Nomor,Referensi,Account,AccountName,Keterangan,Debet,Kredit,Kelompok,CostCenter,Nopol,Cabang';
        QueryToDBGrid(cxGrid1DBTableView1, s,skolom ,dsJurnal);
        ...
        begin
          if cdsjurnal.Fields[i].DataType = ftFloat then
          begin
             cdsjurnal.Fields[i].Alignment := taRightJustify;
             TFloatField(cdsjurnal.Fields[i]).DisplayFormat := '###,###,###';
          end;
        ...
        cxGrid1DBTableView1.Columns[8].Summary.FooterFormat:='###,###,###,###';
          ...
          TcxDBPivotHelper(cxPivot).LoadFromCDS(cdsjurnal);
...
begin
  inherited;
  If cdsjurnal.FieldByname('Nomor').IsNull then exit;
  if ActiveMDIChild.Caption <> 'Invoice' then
   begin
//      ShowForm(TfrmBrowseBarang).Show;
      frmInvoice  := frmmenu.ShowForm(TfrmInvoice) as TfrmInvoice;
      frmInvoice.ID := cdsjurnal.FieldByname('Nomor').AsString;
      frmInvoice.FLAGEDIT := True;
      frmInvoice.edtnOMOR.Text := cdsjurnal.FieldByname('Nomor').AsString;
      frmInvoice.loaddataall(cdsjurnal.FieldByname('Nomor').AsString);
