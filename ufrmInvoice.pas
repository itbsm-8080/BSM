...
type
  ...
  function gettop(akode:String):integer;
    ...
    procedure chkDTPClick(Sender: TObject);
  private
...
begin
  ...
  begin
   ...
   zAddField(FCDS, 'IDBatch', ftString, False, 10);
...
begin
  ...
  s :=  ' insert into TInv_HDR '
             ...
             + ' Inv_disc_fakturpr,Inv_amount,Inv_taxamount,Inv_istax,inv_isdtp,inv_freight,inv_tglbukti,date_create,user_create) '
             + ' values ( '
             + Quot(edtNomor.Text) + ','
...
ExecSQLDirect(frmMenu.conn, s);
  ...
  begin
   ...
   begin
    S:='insert into tinv_dtl (invd_inv_nomor,invd_brg_kode,invd_brg_satuan,invd_idbatch,invd_qty,invd_discpr,invd_harga,invd_nourut,invd_otorisasi,invd_expired) values ('
      + Quot(edtNomor.Text) +','
...
begin
  ...
  s := ' select bpb_nomor,bpb_tanggal,po_sup_kode,po_istax,sup_top,'
     ...
     + ' pod_harga,pod_discpr,(bpbd_qty*pod_harga*(100-pod_discpr)/100) nilai,bpbd_idbatch'
         ...
         begin
            ...
            while  not Eof do
             begin
                      ...
                      CDS.FieldByName('idbatch').AsString      := fieldbyname('bpbd_idbatch').Asstring;
...
begin
  ...
  s := ' select inv_NOMOr,inv_nobukti,inv_tanggal,bpb_nomor,inv_memo,po_sup_kode,inv_istax,inv_isdtp,inv_jthtempo,'
     + ' invd_brg_kode,invd_bRG_satuan,invd_qty,invd_harga,invd_discpr,(invd_qty*invd_harga*(100-invd_discpr)/100) nilai,'
     + ' invd_expired,inv_disc_faktur,inv_disc_fakturpr,inv_freight,inv_tglbukti,invd_idbatch '
   ...
   try
       ...
       begin
         ...
         begin
            ...
            if fieldbyname('inv_isdtp').AsInteger =1 then
               chkdtp.Checked := True
            else
            ...
            while  not Eof do
             begin
                     ...
                     CDS.Append;
                      ...
                      CDS.FieldByName('idbatch').AsString      := fieldbyname('invd_idbatch').Asstring;
...
begin
  ...
  if (chkPajak.Checked) AND (not chkDTP.Checked) then
  begin
...
procedure TfrmInvoice.chkDTPClick(Sender: TObject);
begin
hitung;
end;
