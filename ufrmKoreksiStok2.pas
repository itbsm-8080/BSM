...
begin
   ...
   anomor := edtNomor.Text;
      s:= 'update tkor_hdr set  '
         ...
         + ' korh_gdg_kode = ' + Quot(CDS.FieldByName('GUDANG').asstring)+','
...
begin
  ...
  sqlBantuan:='select distinct brg_kode Kode,brg_nama Nama,brg_satuan Satuan from tbarang inner join tbarangkomposisi on bk_brg_kode=brg_kode';
  Application.CreateForm(Tfrmbantuan,frmbantuan);
  frmBantuan.SQLMaster := SQLbantuan;
  frmBantuan.ShowModal;
   if varglobal <> '' then
...
var
  ...
  aqty : double;
begin
     If cxLookupGudangAsal.EditValue = null then
    begin
      ...
      begin
        try
          ...
          begin
              ...
              CDS.FieldByName('namagudang').AsString := getnama('tgudang','gdg_kode',cxlookupgudangasal.editvalue,'gdg_nama');
              CDS.Post;
...
procedure TfrmKoreksiStok2.cxButton3Click(Sender: TObject);
var
  s:String;
  tt : TStrings;
  i:integer;
begin
  ...
  CDS.First;
  tt := TStringList.Create;
  s:='delete from tbarangkomposisi where bk_brg_kode='+Quot(edtkodebarang.Text)+';';
  tt.append(s);
  while not CDS.Eof do
  begin
   if CDS.FieldByName('qty').asfloat <  0 then
   begin
      s:='insert into tbarangkomposisi (bk_brg_kode,bk_bhn_kode,bk_qty,bk_satuan) values ('
      + Quot(edtkodebarang.Text)+','
      + Quot(cds.fieldbyname('sku').asstring)+','
      + floattostr(cds.fieldbyname('qty').asfloat*-1/strtofloat(edtqty.Text))+','
      + Quot(cds.fieldbyname('satuan').asstring)+');';
      tt.append(s);
   end;
   cds.Next;
