...
var
  ...
  atotal :Double;
begin
  ssql := 'select *,nilai2/nilai*100 ratio from (select year(jur_tanggal) Tahun,month(jur_tanggal) bulan,'
+ ' kol_id,rek_kode ,keterangan,rek_nama Akun,'
+ ' sum(ifnull(jurd_debet,0)-ifnull(jurd_kredit,0)) Nilai, '
+ ' (select sum(ifnull(jurd_debet,0)-ifnull(jurd_kredit,0)) Nilai2 from tjurnalitem inner join tjurnal on jurd_jur_no=jur_no'
+ ' where  (year(jur_tanggal)<'+ edtTahun2.Text
+ ' OR (month(jur_tanggal)<='+IntToStr(cbbBulan2.ItemIndex+1)
+ ' AND year(jur_tanggal)='+ edtTahun2.Text+ ' )) and jurd_rek_kode=y.jurd_rek_kode) Nilai2 '
+ ' from  tjurnal x inner join tjurnalitem y'
...
+ ' group by '
+ ' rek_kode,kol_id'
+ ' order by kol_id,rek_nama) final';
        ds3.Close;
  ...
  ssql := 'select *,nilai2/nilai*100 ratio from (select year(jur_tanggal) Tahun,month(jur_tanggal) bulan,'
+ ' kol_id,rek_kode ,keterangan,rek_nama Akun,sum(ifnull(jurd_debet,0)-ifnull(jurd_kredit,0)) Nilai,'
+ ' (select sum(ifnull(jurd_debet,0)-ifnull(jurd_kredit,0)) Nilai2 from tjurnalitem inner join tjurnal on jurd_jur_no=jur_no'
+ ' where  (year(jur_tanggal)<'+ edtTahun2.Text
+ ' OR (month(jur_tanggal)<='+IntToStr(cbbBulan2.ItemIndex+1)
+ ' AND year(jur_tanggal)='+ edtTahun2.Text+ ' )) and jurd_rek_kode=y.jurd_rek_kode) Nilai2 '
