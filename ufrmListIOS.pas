...
begin
      ...
      s:= ' select Nomor,Tanggal,JthTempo,Kode,Supplier,Tax,'
           + ' NoBukti,Total,ifnull(retur,0) Retur,inv_bayar Bayar,Sisa_Hutang-ifnull(retur,0) Sisa_Hutang,'
           + ' Range_overdue,Overdue,NoPajak,inv_tglbukti Tgl_Supplier from '
           ...
           + ' inv_nobukti NoBukti,inv_tglbukti,inv_amount Total,inv_amount-inv_bayar Sisa_Hutang,inv_bayar, '
           + ' (select sum(ret_amount) from tret_hdr where ret_inv_nomor =z.inv_nomor) Retur, '
           + ' datediff('+quotd(startdate.date)+',inv_jthtempo) Overdue,inv_nopajak NoPajak'
           + ' from tinv_hdr z'
...
//
        Skolom :='Nomor,Tanggal,JthTempo,Supplier,Tax,Nobukti,Total,Retur,Bayar,Sisa_Hutang,Overdue,Range_Overdue,NoPajak,Tgl_Supplier';
