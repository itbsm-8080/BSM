...
begin
      ...
      s:= 'select brg_kode Kode,brg_nama Nama,Kategori, ifnull(hsaldoawal,0) HAwal,ifnull(saldoawal,0) Awal,'
+ ' ifnull(qty_Beli,0) qty_Beli,ifnull(H_Beli,0) H_Beli, ifnull(qty_MTC_01,0) qty_MTC_01,ifnull(H_MTC_01,0) H_MTC_01,'
+ ' ifnull(qty_MTC_02,0) qty_MTC_02,ifnull(H_MTC_02,0) H_MTC_02,'
+ ' ifnull(qty_MTCI_01,0) qty_MTCI_01,ifnull(H_MTCI_01,0) H_MTCI_01, ifnull(qty_MTCI_02,0) qty_MTCI_02,ifnull(H_MTCI_02,0) H_MTCI_02, '
+ ' ifnull(qty_Ret,0) qty_Ret,ifnull(H_Ret,0) H_Ret, ifnull(akhir,0) Akhir,'
+ ' ifnull(hsaldoawal,0)+ifnull(H_Beli,0)+ifnull(H_MTCI_02,0)+ifnull(H_MTCI_01,0)-ifnull(H_MTC_02,0)-ifnull(H_MTC_01,0)-ifnull(H_Ret,0) HAkhir'
+ ' from ( select brg_kode,brg_nama,ktg_nama kategori,brg_hrgbeli, brg_lastcost,'
+ ' (select sum(mst_stok_in-mst_stok_out) from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal < '+QuotD(startdate.DateTime)+') saldoawal,'
+ ' (select sum((mst_stok_in-mst_stok_out) *mst_hargabeli) from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal < '+QuotD(startdate.DateTime)+') Hsaldoawal,'
+ ' (select sum(mst_stok_out) from tmasterstok inner join tmutcab_hdr on mutc_nomor=mst_noreferensi where mst_brg_kode=a.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime)
+ ' and mst_noreferensi like "%MTC%" and mutc_cbg_tujuan="01" )  qty_MTC_01,'
+ ' (select sum(mst_stok_out*mst_hargabeli) from tmasterstok inner join tmutcab_hdr on mutc_nomor=mst_noreferensi  where mst_brg_kode=a.brg_kode'
+ ' and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%MTC%" and mutc_cbg_tujuan="01")   H_MTC_01,'
+ ' (select sum(mst_stok_out) from tmasterstok inner join tmutcab_hdr on mutc_nomor=mst_noreferensi where mst_brg_kode=a.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime)
+ ' and mst_noreferensi like "%MTC%" and mutc_cbg_tujuan="02" )  qty_MTC_02,'
+ ' (select sum(mst_stok_out*mst_hargabeli) from tmasterstok inner join tmutcab_hdr on mutc_nomor=mst_noreferensi  where mst_brg_kode=a.brg_kode'
+ ' and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%MTC%" and mutc_cbg_tujuan="02")   H_MTC_02,'
+ ' (select sum(mst_stok_out)'
+ ' from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%RET%")  qty_Ret,'
+ ' (select sum(mst_stok_out*mst_hargabeli) from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal'
+ ' between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%RET%")  H_Ret,'
+ ' (select sum(mst_stok_in-mst_stok_out)'
+ ' from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%RI%")  qty_Beli,'
+ ' (select sum((mst_stok_in-mst_stok_out)*mst_hargabeli) from tmasterstok where mst_brg_kode=a.brg_kode and mst_tanggal'
+ ' between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%RI%")  H_Beli,'
+ ' (select sum(mst_stok_in) from tmasterstok'
+ ' inner join tmutcabIN_hdr on mutcI_nomor=mst_noreferensi where mst_brg_kode=a.brg_kode and mst_tanggal'
+ ' between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%MTCI%" and mutci_cbg_asal="01" )  qty_MTCI_01,'
+ ' (select sum(mst_stok_in*mst_hargabeli) from tmasterstok inner join tmutcabin_hdr on mutci_nomor=mst_noreferensi'
+ ' where mst_brg_kode=A.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi'
+ ' like "%MTCI%" and mutcI_cbg_asal="01")   H_MTCI_01,'
+ ' (select sum(mst_stok_in) from tmasterstok'
+ ' inner join tmutcabIN_hdr on mutcI_nomor=mst_noreferensi where mst_brg_kode=a.brg_kode and mst_tanggal'
+ ' between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi like "%MTCI%" and mutci_cbg_asal="02" )  qty_MTCI_02,'
+ ' (select sum(mst_stok_in*mst_hargabeli) from tmasterstok inner join tmutcabin_hdr on mutci_nomor=mst_noreferensi'
+ ' where mst_brg_kode=a.brg_kode and mst_tanggal between '+quotd(startdate.DateTime)+' and '+quotd(enddate.DateTime) + ' and mst_noreferensi'
+ ' like "%MTCI%" and mutcI_cbg_asal="02")   H_MTCI_02,'
+ ' (select sum(mst_stok_in-mst_stok_out) from tmasterstok'
+ ' where mst_brg_kode=a.brg_kode and mst_tanggal <= '+quotd(enddate.DateTime)+') akhir'
+ ' from tbarang a  left join tkategori'
+ ' on ktg_kode=brg_ktg_kode ) final';
...
//
        Skolom :='Kode,Nama,Kategori,Awal,HAwal,Qty_Beli,H_Beli,Qty_MTC_01,H_MTC_01,Qty_MTC_02,H_MTC_02,Qty_MTCI_01,H_MTCI_01,Qty_MTCI_02,H_MTCI_02,Qty_Ret,H_Ret,Akhir,HAkhir';
