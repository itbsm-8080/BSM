  ...
  begin
   ...
   begin
        s:='insert into tmutasi_dtl '
          + ' (mutd_mut_nomor,mutd_brg_kode,mutd_qty,mutd_keterangan,mutd_expired,mutd_nourut) values ( '
          +  Quot(anomor) + ','
...
begin
  ...
  try
    ftsreport.Nama := 'mutasigdg';
       ...
       + ' left join  tmutasi_dtl b on mut_nomor=mutd_mut_nomor and e.tam_nama=b.mutd_brg_kode  and e.expired=mutd_expired'
...
procedure TfrmMutasiGudang.edtKodeBarangClickBtn(Sender: TObject);
var
  s:String;
  tsql:TmyQuery;
  aqty : double;
...
procedure TfrmMutasiGudang.Button4Click(Sender: TObject);
VAR
  aqty :double;
  s:string;
  tsql:TmyQuery;
...
s:='select  brg_kode,brg_nama,brg_satuan,bk_qty,brg_lastcost ,'
      ...
      + ' (select mst_expired_date from tmasterstok where mst_brg_kode=bk_bhn_kode limit 1) expired '
      ...
      begin
        try
          ...
          begin
              ...
              CDS.FieldByName('qTY').asfloat :=Fields[3].Asfloat*aqty;
