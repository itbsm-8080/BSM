...
uses
  ...
  frxClass, frxDMPExport, dxSkinBlack, dxSkinBlue, dxSkinCaramel,
...
function TfrmPO.getmaxkode2:string;
var
  s:string;
begin

  s:='select max(right(po_nomor,4)) from tpo_hdr where po_nomor like ' + quot(NOMERATOR+'.'+FormatDateTime('yymm',Date)+'.%')
  + ' and po_istax=1 ';
  with xOpenQuery(s,frmMenu.conn) do
  begin
    try
...
begin
  ...
  begin
    S := 'select sup_nama as supplier, sup_kode Kode, sup_alamat Alamat,sup_telp'
        +' from tsupplier where sup_isaktif=1 order by sup_nama';
...
begin
  ...
  S := 'select brg_kode sku, brg_nama NamaBarang, brg_hrgbeli HrgBeli,'
      + ' brg_satuan Satuan from tbarang where brg_isaktif=1 ORDER BY brg_nama';
...
function Tfrmpo.getstok(asku:string): Double;
var
...
begin
  Result := 0;
  S:= 'select sum(mst_stok_in-mst_stok_out) from tmasterstok where mst_gdg_kode="WH-00" AND mst_brg_kode='+Quot(asku) ;
...
function Tfrmpo.getavgsales(asku:string): Double;
var
...
begin
  Result := 0;
  S:= 'SELECT ifnull(SUM(mst_stok_out),0) /3 '
+ ' FROM tmasterstok '
...
begin
  ...
  s :=  ' insert into Tpo_HDR '
             ...
             + ' po_disc_fakturpr,po_amount,po_taxamount,po_istax,po_isdtp,po_dateline,po_pemesan,date_create,user_create) '
             + ' values ( '
             + Quot(edtNomor.Text) + ','
...
begin
  ...
  s := ' select po_nomor,po_tanggal,po_memo,po_sup_kode,po_pemesan,'
     + ' po_DISC_faktur,po_disc_fakturpr,po_istax,po_isdtp,pod_keterangan,'
   ...
   try
       ...
       begin
         ...
         begin
            ...
            if fieldbyname('po_isdtp').AsInteger =1 then
               chkdtp.Checked := True
            else
...
procedure TfrmPO.chkDTPClick(Sender: TObject);
begin
hitung;
end;
