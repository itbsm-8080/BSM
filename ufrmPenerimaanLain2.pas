...
ExecSQLDirect(frmMenu.conn, s);
   ...
   // insert detail di header
          ...
          +  Quot(anomor) + ','
          +  Quot(VarToStr(cxLookupAccount.EditValue)) + ',0,'
   ...
   try
       ...
       begin
         ...
         begin
           ...
           cxLookupAccount.EditValue := fieldbyname('jurd_rek_kode').AsString;
...
var
  s:string;
begin
  s:='select max(right(jur_no,4)) from tjurnal  where jur_no like ' + quot(NOMERATOR+'.'+FormatDateTime('yymm',dtTanggal.Date)+'.%') ;
...
begin
  ...
  result:=true;
   i := 1;
   If cxlookupaccount.EditValue = '' then
    begin
