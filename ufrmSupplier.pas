...
begin
  try
    ...
    begin
      ...
      edtUpHet.Text := fieldbyname('sup_Uphet').AsString;
//      edthutang.text :=formatfloat('###,###,###.##',fieldbyname('sup_hutang').Asfloat);
      ...
      If fieldbyname('sup_isaktif').AsInteger =1 then
         chkAktif.Checked := True
      else
...
if FLAGEDIT then
  s:='update tsupplier set '
    ...
    + ' sup_UpHET = ' + edtUpHet.Text + ','
    + ' sup_isaktif = '+ IntToStr(isaktif) + ','
...
//  edtKode.Text := getmaxkode;
  s :=  ' insert into tsupplier '
             ...
             + ' sup_rekening,sup_bank,sup_cabang,sup_atasnama,sup_UpHET,sup_isaktif,date_create,user_create) '
             + ' values ( '
             + Quot(edtKode.Text) + ','
             + Quot(edtNama.Text) + ','
             + Quot(edtkota.Text)+','
